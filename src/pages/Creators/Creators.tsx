import "./Creators.css";

const Creators = () => {
  return (
    <>
      <h1>
        This website was created by students from the Highschool Kiro Spandzov
        Brko - Kavadarci, Macedonia
      </h1>
      <h1>
        Original website created by us for national MASUM competition 2023 using
        plain HTML, CSS and JS
      </h1>
      <h1>Project ported to React on 05.2023</h1>
      <h1>Team members : </h1>
      <a href="https://github.com/DimeDimitrov">Dimitar Dimitrov</a>
      <br />
      <a href="https://github.com/FrozenXE">Jovan Mitrev</a>
      <br />
      <a href="https://github.com/Stefchee">Stefan Panov</a>
      <br />
      <a href="https://github.com/Suddke">
        Petar Stefanov (Currently no commits as of React port)
      </a>
      <br />

      <h1>All rights reserved by original creators</h1>
    </>
  );
};

export default Creators;
